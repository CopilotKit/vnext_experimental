...
describe('Frontend Tool Integration', () => {
  ...
  describe('Render Tool Calls State', () => {
    it('should update current render tool calls', () => {
      ...
      const renders = {
        tool1: { args: z.object({}), render: {} as any },
        tool2: { args: z.object({}), render: {} as any }
      };
    ...
    it('should merge renders when registering', () => {
      service.setCurrentRenderToolCalls({
        existing: { args: z.object({}), render: {} as any }
      });
      ...
      expect(Object.keys(renders)).toContain('existing');
      expect(Object.keys(renders)).toContain('newTool');
    });

    it('should preserve other renders when unregistering', () => {
      service.setCurrentRenderToolCalls({
        keep1: { args: z.object({}), render: {} as any },
        remove: { args: z.object({}), render: {} as any },
        keep2: { args: z.object({}), render: {} as any }
      });

      service.unregisterToolRender('remove');

      const renders = service.currentRenderToolCalls();
      expect(Object.keys(renders)).toEqual(['keep1', 'keep2']);
    });
  });
});