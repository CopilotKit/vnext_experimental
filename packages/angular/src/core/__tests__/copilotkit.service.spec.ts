...
import { effect, runInInjectionContext, Injector, Component } from '@angular/core';
...
describe('CopilotKitService', () => {
  ...
  describe('Reactivity - Signal Updates', () => {
    ...
    it('should trigger computed signal updates when dependencies change', () => {
      ...
      service.setCurrentRenderToolCalls([{ name: 'test', args: {} as any, render: {} as any }]);
      
      // Get new context value
      contextValue = service.context();
      expect(contextValue.currentRenderToolCalls).toEqual([{ name: 'test', args: {}, render: {} }]);
    });
  ...
  describe('Reactivity - Observable Updates', () => {
    ...
    it('should emit context changes through context$', async () => {
      ...
      // Trigger a change
      service.setCurrentRenderToolCalls([{ name: 'newTool', args: {} as any, render: {} as any }]);
      
      // Wait for observable emission
      await new Promise(resolve => setTimeout(resolve, 0));
      
      const lastContext = contexts[contexts.length - 1];
      expect(lastContext.currentRenderToolCalls).toEqual([{ name: 'newTool', args: {}, render: {} }]);
  ...
  describe('Frontend Tools Support', () => {
    it('should process frontend tools correctly', () => {
      ...
      expect(serviceWithTools.copilotkit.tools['calculate'].name).toBe('calculate');
    });
    ...
    it('should handle frontend tools with render components', () => {
      ...
      const toolRender = renderToolCalls.find(r => r.name === 'toolWithRender');
  ...
  describe('Human-in-the-Loop Support', () => {
    it('should process human-in-the-loop tools correctly', () => {
      ...
      expect(serviceWithTools.copilotkit.tools['requestApproval']).toBeDefined();
      expect(serviceWithTools.copilotkit.tools['requestApproval'].name).toBe('requestApproval');
    });
    ...
    it('should add render components for human-in-the-loop tools', () => {
      ...
      const renderToolCalls = serviceWithTools.renderToolCalls();
      const confirmRender = renderToolCalls.find(r => r.name === 'confirmAction');
  ...
  describe('Agent ID Constraints', () => {
    it('should handle frontend tools with agentId', () => {
      ...
      expect(serviceWithTools.copilotkit.tools['globalTool'].agentId).toBeUndefined();
      expect(serviceWithTools.copilotkit.tools['agent1Tool'].agentId).toBe('agent1');
      expect(serviceWithTools.copilotkit.tools['agent2Tool'].agentId).toBe('agent2');
    });
    ...
    it('should handle render tool calls with agentId', () => {
      ...
      const renderToolCalls = serviceWithTools.renderToolCalls();
      const globalRender = renderToolCalls.find(r => r.name === 'globalRender');
      const agent1Render = renderToolCalls.find(r => r.name === 'agent1Render');
    ...
    it('should handle frontend tools with render and agentId', () => {
      ...
      expect(serviceWithTools.copilotkit.tools['agentSpecificTool']).toBeDefined();
      expect(serviceWithTools.copilotkit.tools['agentSpecificTool'].agentId).toBe('specificAgent');
      
      // Check render is registered with agentId
      const renderToolCalls = serviceWithTools.renderToolCalls();
      const agentRender = renderToolCalls.find(r => r.name === 'agentSpecificTool');
    ...
    it('should handle human-in-the-loop tools with agentId', () => {
      ...
      expect(serviceWithTools.copilotkit.tools['agentApproval'].agentId).toBe('approvalAgent');
      
      // Check render is registered with agentId
      const renderToolCalls = serviceWithTools.renderToolCalls();
      const approvalRender = renderToolCalls.find(r => r.name === 'agentApproval');
    ...
    it('should handle mixed tools with and without agentId', () => {
      ...
      expect(serviceWithTools.copilotkit.tools['globalTool'].agentId).toBeUndefined();
      expect(serviceWithTools.copilotkit.tools['specificTool'].agentId).toBe('specificAgent');
      expect(serviceWithTools.copilotkit.tools['hitlTool'].agentId).toBe('hitlAgent');
  ...
  describe('Combined Tools and Renders', () => {
    it('should combine all tools and render calls correctly', () => {
      ...
      const renderToolCalls = serviceWithTools.renderToolCalls();
      const frontendRender = renderToolCalls.find(r => r.name === 'frontendTool');
      const hitlRender = renderToolCalls.find(r => r.name === 'hitlTool');
