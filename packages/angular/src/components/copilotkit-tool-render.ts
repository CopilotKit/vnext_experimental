...
@Component({
  ...
  template: `
    ...
    @if (templateRef && templateContext) {
      <ng-container
...
export class CopilotKitToolRender implements OnChanges, AfterViewInit {
  toolName = input.required<string>();
  args = input<any | undefined>(undefined);
  status = input<ToolCallStatus>(ToolCallStatus.InProgress);
  result = input<any | undefined>(undefined);
