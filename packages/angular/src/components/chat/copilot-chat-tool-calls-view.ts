 ...
 * using the registered render functions in CopilotKit.
 */
@Component({
  standalone: true,
  ...
  template: `
    @for (toolCall of message()?.toolCalls ?? []; track toolCall.id) {
      <ng-container>
        ...
        @if (getTemplateForToolCall(toolCall); as templateData) {
          <ng-container
            [ngTemplateOutlet]="templateData.template"
            [ngTemplateOutletContext]="templateData.context"
          ></ng-container>
        }
      </ng-container>
    }
  ...
  implements AfterViewInit, OnChanges, OnDestroy
{
  ...
  private copilotkit: CopilotKit | null = inject(CopilotKit, {
    optional: true,
  });
