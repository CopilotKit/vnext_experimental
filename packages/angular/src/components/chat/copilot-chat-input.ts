...
export class CopilotChatInput implements AfterViewInit, OnDestroy {
  ...
  computedValue = computed(() => {
    ...
    const configValue =
      this.chatConfig?.inputValue?.() ?? this.chatConfig?.inputValue?.();
  ...
  handleValueChange(value: string): void {
    ...
    if (this.chatConfig?.setInputValue) this.chatConfig.setInputValue(value);
  }
  ...
  send(): void {
    ...
    if (trimmed) {
      ...
      // Clear input via chat config and bound textarea
      if (this.chatConfig?.setInputValue) this.chatConfig.setInputValue("");
