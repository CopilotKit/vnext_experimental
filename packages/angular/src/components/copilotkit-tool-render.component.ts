...
import type { ToolCallProps, AngularToolCallRender } from '../core/copilotkit.types';
...
export class CopilotkitToolRenderComponent implements OnChanges, AfterViewInit {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['toolName'] || changes['args'] || changes['status'] || changes['result']) {
      this.renderTool();
    }
  }
  ...
  private renderTool(): void {
    ...
    const toolRender = this.copilotkit.getToolRender(this.toolName) as AngularToolCallRender | undefined;
  ...
  private renderComponent(componentClass: Type<any>, props: ToolCallProps): void {
    // Clear the container
    ...
    // Check if component expects individual inputs or a single props object
    if ('name' in instance || 'args' in instance || 'status' in instance) {
      // Component expects individual inputs
      Object.assign(instance, props);
    } else if ('props' in instance) {
      // Component expects a single props object
  ...
  private renderTemplate(template: TemplateRef<any>, props: ToolCallProps): void {
