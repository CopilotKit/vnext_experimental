import { Middleware, RunAgentInput, AbstractAgent, BaseEvent, Tool, EventType, Message, ToolCall, ToolCallResultEvent, ActivitySnapshotEvent, RunStartedEvent, RunFinishedEvent } from "@ag-ui/client";
...
/**
 * Configuration for MCPAppsExtensionMiddleware
 */
export interface MCPAppsExtensionMiddlewareConfig {
  /**
...
/**
 * MCP Apps Extension middleware - fetches UI-enabled tools from MCP servers.
 */
export class MCPAppsExtensionMiddleware extends Middleware {
  private config: MCPAppsExtensionMiddlewareConfig;
  /** Map of tool name -> server config for UI tools */
  private uiToolsMap: Map<string, MCPClientConfig> = new Map();

  constructor(config: MCPAppsExtensionMiddlewareConfig = {}) {
    super();
    this.config = config;
  }
  ...
  run(input: RunAgentInput, next: AbstractAgent): Observable<BaseEvent> {
    ...
    const proxiedRequest = input.forwardedProps?.__proxiedMCPRequest as ProxiedMCPRequest | undefined;
    if (proxiedRequest) {
   ...
   */
  private handleProxiedMCPRequest(runId: string, request: ProxiedMCPRequest): Observable<BaseEvent> {
   ...
   */
  private async executeMCPRequest(request: ProxiedMCPRequest): Promise<unknown> {
    let transport;
    ...
    try {
      ...
      switch (request.method) {
        case "tools/call":
          return await client.callTool(request.params as { name: string; arguments?: Record<string, unknown> });
        case "resources/read":
        ...
        case "notifications/message":
          ...
          await client.notification({ method: "notifications/message", params: request.params });
          return { success: true };
        case "ping":
          return await client.ping();
        default:
          throw new Error(`MCP method not allowed for UI proxy: ${request.method}`);
      }
    } finally {
      await client.close();
    }
            ...
            try {
              ...
              for (const toolCall of pendingUIToolCalls) {
                ...
                try {
                  ...
                  const mcpResult = await this.executeToolCall(toolInfo.serverConfig, toolCall.function.name, args);

                  // Fetch the UI resource
                  const resource = await this.readResource(toolInfo.serverConfig, toolInfo.resourceUri);
                  ...
                  console.error(`Failed to execute UI tool call ${toolCall.function.name}:`, error);
                  // Emit error result
  ...
  private async fetchUITools(): Promise<UIToolInfo[]> {
    ...
    for (const serverConfig of this.config.mcpServers || []) {
      try {
        ...
        console.error(`Failed to fetch tools from MCP server ${serverConfig.url}:`, error);
      }
    }
   ...
   */
  private async fetchToolsFromServer(serverConfig: MCPClientConfig): Promise<UIToolInfo[]> {
    let transport;
