...
The hook accepts a `ReactHumanInTheLoop` object plus an optional React dependency array as the second argument. Dependencies memoize the tool’s render component so you can tie it to component state without wrapping the tool in `useMemo`.
