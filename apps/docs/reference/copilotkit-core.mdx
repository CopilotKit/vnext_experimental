...
- `agents`: `Readonly<Record<string, AbstractAgent>>` A combined view of all available agents, merging local development
  agents with those discovered from the runtime. This is read-only to prevent accidental modifications.
...
CopilotKitCore provides two primary methods for executing agents: `runAgent` for immediate execution with message handling,
and `connectAgent` for establishing live connections to existing agent sessions.
