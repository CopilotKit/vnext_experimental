...
import createEndpoint from "@copilotkitnext/runtime/endpoint";
...
| `/agent/{agentId}/run` | POST   | Execute a specific agent             |
...
} from "@copilotkitnext/runtime/endpoint";
...
switch (requestType) {
  ...
  case CopilotKitRequestType.RunAgent:
    // POST /agent/{agentId}/run - Run agent
...
```typescript
const secureRuntime = new CopilotRuntime({
  agents: myAgents,
